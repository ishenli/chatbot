...
public class ChatMessageItemVO {
    ...
    public ChatMessageItemVO(String name, String message, String avatar) {
        this.id = "haomo";
        this.name = name;
    ...
    public static  ArrayList<ChatMessageItemVO> mock() {
        int arraySize = 10; // 假设我们要生成一个包含10个ChatListItemVO对象的数组
        ArrayList<ChatMessageItemVO> persons = new ArrayList<>(arraySize);
        Random random = new Random();
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            int nameLength = random.nextInt(30) + 10; // nextInt(16)生成0-15，加上5变为5-20
            ...
            String name = nameBuilder2.toString();
            String message = nameBuilder.toString();
            String avatar = list.get(nameLength2);
            ChatMessageItemVO person = new ChatMessageItemVO(name, message, avatar);
            persons.add(person);
        }
